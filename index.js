...
function V(message, args) {
	var embed = new Discord.RichEmbed()
	.setColor(color.color)
	.setAuthor(args, "https://cdn.pixabay.com/photo/2015/02/27/06/29/sound-651706_960_720.png");
	message.channel.sendEmbed(embed);
}
...
function VS(message, args, num) {
	var embed = new Discord.RichEmbed()
	.setColor(color.color)
	.setDescription(args)
	.setFooter(`Requires ${num} Votes.`);
	message.channel.sendEmbed(embed);
}
...
function Stop(message, args) {
	var embed = new Discord.RichEmbed()
	.setColor(color.color)
	.setAuthor(args, "https://cdn4.iconfinder.com/data/icons/social-messaging-ui-coloricon-1/21/34-512.png")
	message.channel.sendEmbed(embed);
}
...
client.on('message', async message => { 
					...
					message.channel.send({embed : takoota}).then(async msg1 => {
						...
						t.on("collect",async () => { 
							if (pages == 1 && videos.length > 10) {
								...
								.setThumbnail("https://www.denverlibrary.org/sites/dplorg/files/2018/02/youtube-344105_640.png")
								.setDescription(`${videos.map(video2 => `${++index}. **${video2.title}**\n`).slice(10,20).join('\n')}`)
								.setFooter(`Page 2 of ${nPage}`);
								msg1.edit({embed : takoota});
							} else if (pages == 2 && videos.length > 20) {
								pages+=1;
								takoota = new Discord.RichEmbed()
								.setAuthor("Search results..", 'https://cdn.discordapp.com/attachments/480852478196187138/483165667877453834/20180826_094749.png')
								.setColor(color.color) 
								.setThumbnail("https://www.denverlibrary.org/sites/dplorg/files/2018/02/youtube-344105_640.png")
	...
	if (!serverQueue) {
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/) || args[1].includes('playlist')) return;
	...
	return undefined;
		...
		if (required<=serverQueue.vote.length) {
			...
			VS(message, `**${message.author.username}**, has been voted to skip`, required);
		}
		return undefined;
	...
	if (!message.member.voiceChannel) return err(message,`You Should Be in A Voice Channel To use This Command.`);		
		if (message.guild.members.get(message.member.id).voiceChannel.id !== message.guild.members.get(client.user.id).voiceChannel.id) return err(message , `You Should Be in My Voice Channel To Use My Commands.`);
		sendMessageQueue = false;
		Stop(message, `Music has been stopped.`);
		serverQueue.songs = [];
		serverQueue.connection.dispatcher.end('Stop command has been used!');
		setTimeout(() => sendMessageQueue = true, 1000);
	} else if (command === "leave" || command === 'disconnect' || command === 'l') {
		if (client.user.username.includes('Alpha')) return;
		...
		setTimeout(() => sendMessageQueue = true, 1000);
		leave(message, `:door: I have disconnected from ${message.guild.members.get(client.user.id).voiceChannel.name}`);
		...
		Vs(message, `Volume Successfully Changed from ${serverQueue.volume}% to ${args[1]}%`);
	...
	} else if (command === 'queue-remove' || command === 'q-r' || command === 'queue-r' || command === 'queue-rem') {
		if (!serverQueue) return err(message,`You Should Play Something To Use This Command.`);
		else if (!message.member.voiceChannel) return err(message,`You Should Be in A Voice Channel To Use This Command.`);
		else if (message.guild.members.get(message.member.id).voiceChannel.id !== message.guild.members.get(client.user.id).voiceChannel.id) return err(message , `You Should Be in My Voice Channel To Use My Commands.`);
		else if (serverQueue.songs.length == 1) return err(message, `Only 1 song is queued.`);
		else {
			if (!args[1]) return err(message, 'You Should To Select A Song Number.');
			else if (isNaN(args[1])) return err(message, 'Only numbers are allowd.');
			else if(args[1] < 1) return err(message, `Only allowed from 1 - ${serverQueue.songs.length - 1}`);
			else if (args[1] > serverQueue.songs.length - 1) return err(message, `Only ${serverQueue.songs.length - 1} song is queued.`);
			e(message, `**${serverQueue.songs[args[1]].title}**, has been removed from the queue`);
			serverQueue.songs.splice(args[1], 1);
		}
	} else if (command === 'np' | command === 'nowplay' | command === 'nowplaying' | command === 'now-playing') {
		if (!serverQueue) return err(message,`You Should Play Something To Use This Command.`);
		else if (!message.member.voiceChannel) return err(message,`You Should Be in A Voice Channel To Use This Command.`);
		else if (message.guild.members.get(message.member.id).voiceChannel.id !== message.guild.members.get(client.user.id).voiceChannel.id) return err(message , `You Should Be in My Voice Channel To Use My Commands.`);
		else {
			return eee(message, `Now Playing  🎶 ${serverQueue.songs[0].title}`);
		}
...
function play(guild, song, message) {
	setTimeout(function(){
		...
		if (!song) {
			...
			if(sendMessageQueue == true) serverQueue.textChannel.send(embed);
			return;
		}
